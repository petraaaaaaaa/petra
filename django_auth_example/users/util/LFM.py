...
class LFM:
    ...
    def _load_data(self, train_size):
        ...
        for index, row in self.data.iterrows():
            ...
            if user_id not in self.rating_dict.keys():
                self.rating_dict[user_id] = {}
            self.rating_dict[user_id][item_id] = rating
    ...
    def _init_model(self):
        ...
        print(self.user_ids)
        #self.user_ids = set(self.data['user'].values)  # 取出全部user_id
        #self.item_ids = set(self.data['item'].values)  # 取出全部item_id
    ...
    def _get_pos_neg_item(self, user_id):
        ...
        positive_item_ids = set(
            self.train_data[(self.train_data['user'] == user_id) & (self.train_data['rating'] >= 3.5)]['item'])  # 取出包含该user_id的item，用户有过的item
        negative_item_ids = set(
            self.train_data[(self.train_data['user'] == user_id) & (self.train_data['rating'] < 3.5)]['item'])
        # print(positive_item_ids)
        # print(self.item_ids)
        expected_negative_size = self.ratio * len(positive_item_ids)
        if len(negative_item_ids) < expected_negative_size:
            add_negative_item_ids = self.item_ids ^ positive_item_ids  # python求对称差集，返回两个集合不重复的元素。（类变量item_ids中有数据）
            add_negative_item_ids = list(
            np.random.choice(list(add_negative_item_ids), expected_negative_size - len(negative_item_ids)))  # 负样例取多倍
            negative_item_ids = list(negative_item_ids) + add_negative_item_ids
        # print(positive_item_ids)
        # print(negative_item_ids)

        item_dict = {}
        for item_id in positive_item_ids:
            item_dict[item_id] = self.rating_dict[user_id][item_id] / 5.0
        for item_id in negative_item_ids:
            if item_id in self.rating_dict[user_id].keys():
                item_dict[item_id] = self.rating_dict[user_id][item_id] / 5.0
            else:
                item_dict[item_id] = 0.2
        return item_dict
    ...
    def _predict(self, user_id, item_id):
        ...
        r = (p * q).sum()
        sigmod = 1.0 / (1 + np.exp(-r))
        return sigmod

    def _loss(self, user_id, item_id, y, step):
        pred_rating = self._predict(user_id, item_id)
        loss = math.fabs(y - pred_rating)
        print('Step: {}, user_id: {}, item_id: {}, rating: {},  predicted:{}, loss: {}'.
              format(step, user_id, item_id, y, pred_rating, loss))
        return loss
    ...
    def predict(self, user_id, N=None, exclude_rated_items=True):
        """
        根据用户推荐topN结果
        :param user_id:
        :param N:
        ...
        target_item_ids = self.item_ids ^ user_item_ids  if exclude_rated_items else self.item_ids# 求差集，去除用户已选过的item
        interest_list = None
        interest_list = [self._predict(user_id, item_id) for item_id in target_item_ids]
        result_list = sorted(zip(list(target_item_ids), interest_list), key=lambda x: x[1], reverse=True)  # 结果根据评分排序
        if N:
            return result_list[:N]
        else:
            return result_list
    
    def recommend(self, rating_dict, N=20):
        user_item_ids = set(rating_dict.keys())
        target_item_ids = self.item_ids - user_item_ids
        weight_sum = 0
        user_profile = np.array([0 for i in range(self.p.shape[1])])
        for item_id in user_item_ids:
            if item_id in self.item_ids:
                item_profile = np.array(self.q.loc[item_id].values)
            else:
                item_profile = np.array(self.q.loc[0].values)
            weight = rating_dict[item_id] / 5.0
            user_profile = user_profile + weight * item_profile
            weight_sum += weight
        user_profile /= weight_sum
        interest_list = None
        interest_list = [self._predict_score(user_profile, item_id) for item_id in target_item_ids]
        result_list = sorted(zip(list(target_item_ids), interest_list), key=lambda x: x[1], reverse=True)
        if N:
            return result_list[:N]
        else:
            return result_list
    
    def _predict_score(self, user_profile, item_id):
        p = np.mat(user_profile)  # 用户对于目标物品属于哪个类的兴趣查找表
        q = np.mat(self.q.loc[item_id].values).T  # 物品属于哪个类的一个概率
        r = (p * q).sum()
        sigmod = 1.0 / (1 + np.exp(-r))  # sigmod，单位阶跃函数,将兴趣度限定在[0,1]范围内
        return sigmod
    
    def predict_for_rated_items(self, user_id, N):
        """
        根据用户推荐topN结果
        :param user_id:
        :param N:
        ...
        """
        user_item_ids = set(self.train_data[self.train_data['user'] == user_id]['item'])
        interest_list = [self._predict(user_id, item_id) for item_id in user_item_ids]
        result_list = sorted(zip(list(user_item_ids), interest_list), key=lambda x: x[1], reverse=True)  # 结果根据评分排序
        if N:
            return result_list[:N]
        else:
            return result_list
    ...
    def validate(self):
        ...
        for user_id, item_dict in self.items_dict.items():
            ...
            for _, item_rating in enumerate(predict_result):
                ...
                if item_id in self.rating_dict[user_id].keys():
                    absError_sum += abs(self.rating_dict[user_id][item_id] - rating * 5.0)
                    squaredError_sum += (self.rating_dict[user_id][item_id] - rating * 5.0) ** 2
                    setSum += 1
            i += 1
            if i >= 1000:
                break
    ...
    def validate_for_rated_items(self):
        """
        计算MAE, RMSE指标值
        :return:
        """
        ...
        i = 0
        print(self.rating_dict)
        for user_id, item_dict in self.rating_dict.items():
            print(user_id)
            predict_result = self.predict_for_rated_items(user_id, N=None)
            for _, item_rating in enumerate(predict_result):
                item_id = item_rating[0]
                rating = item_rating[1]
                if item_id in self.rating_dict[user_id].keys():
    ...
    def evaluate(self):
        ...
        pred_result = self.predict(len(test_users), N=pred_num, exclude_rated_items=False)
...
if __name__ == '__main__':
    ...
    for i in ratio_range:
        ...
        lfm.load()
        mae, rmse = lfm.validate_for_rated_items()
